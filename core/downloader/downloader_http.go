...
func (this *HttpDownloader) Download(req *context.Request) *page.Page {
	var mtype string
	...
	default:
		log.Error("error request type:" + mtype)
	}
	return p
}
...
func connectByHttpProxy(p *page.Page, in_req *context.Request) (*http.Response, error) {
...
func (this *HttpDownloader) downloadFile(p *page.Page, req *context.Request) (*page.Page, string) {
	var err error
	var urlstr string
...
func (this *HttpDownloader) downloadHtml(p *page.Page, req *context.Request) *page.Page {
	var err error
...
func (this *HttpDownloader) downloadJson(p *page.Page, req *context.Request) *page.Page {
	var err error
	p, destbody := this.downloadFile(p, req)
	if !p.IsSucc() {
		return p
	...
	if r, err = simplejson.NewJson(body); err != nil {
		log.Error(string(body) + "\t" + err.Error())
		p.SetStatus(true, err.Error())
		return p
	}
...
func (this *HttpDownloader) downloadText(p *page.Page, req *context.Request) *page.Page {
	p, destbody := this.downloadFile(p, req)
	if !p.IsSucc() {
		return p
	}
